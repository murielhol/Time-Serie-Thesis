...
def fkl( angles, parent, offset, rotInd, expmapInd, data_utils ):
  """
...
def revert_coordinate_space(channels, R0, T0, data_utils):
  """
...
def run(data_utils):
  ...
  for action in data_utils.actions:
    ...
    expmap_all = revert_coordinate_space( np.vstack((expmap_gt, expmap_pred)), np.eye(3), np.zeros(3) , data_utils)
    ...
    for i in range( nframes_gt ):
      xyz_gt[i,:] = fkl( expmap_gt[i,:], parent, offset, rotInd, expmapInd , data_utils)
    for i in range( nframes_pred ):
      xyz_pred[i,:] = fkl( expmap_pred[i,:], parent, offset, rotInd, expmapInd , data_utils)
