...
class PricePredictor(object):
    ...
    def evaluate(self, x, y, mask_gen):
        ...
        y = Variable(torch.from_numpy(y)).float()
        loss, kld_loss, pred = self._model.gen([x,y, mask_gen]);
    ...
    def _validate(self, steps = 5, epoch=200):
        ...
        f.write('epoch: '+str(mmd.item())+'\n')
        f.write('steps: '+str(mmd.item())+'\n')
    ...
    def _make_figs(self, steps = 5, epoch=500):
        ...
        for seed in range(seeds):
            ...
            for step in range(steps):
                _, _, test_pars = self.evaluate(X[:, step:step+receptive_field, :], y[:, step:step+receptive_field, :], mask)
        ...
        df.to_csv('images/swn_hybrid_samples.csv')
        ...
        df.to_csv('images/swn_hybrid_prediction.csv')