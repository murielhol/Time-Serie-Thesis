...
class PricePredictor(object):
    ...
    def _backtest(self, samples_per_tick=500, view = 1, risk_view=1, epoch=1000, alpha=5):
        ...
        actions_perfect = self.goal(x[:, receptive_field-1:, 0], actions.copy(), D , view, receptive_field)
        R_goal = []
        P_goal = []
        for run in range(runs):
            roi, profit = compute_roi(x[run,receptive_field-1:,0], actions_perfect[run,:], transaction_cost)
            R_goal.append(roi)
            P_goal.append(profit)

        print('GOAL:', np.sum(R_goal), np.sum(P_goal))
        ...
        f.write('run_index: '+str(run_index)+'\n')
        f.write('epoch: '+str(epoch)+'\n')
    ...
    def goal(self, target, actions, D, view, receptive_field):
        ...
        for run in range(np.shape(target)[0]):
            ...
            for tick in range(np.shape(actions)[1]):
                x0 = target[run,tick]
                expected_future = target[run,tick+1:tick+1+view]
                ...
                if not have_state:
                    ...
                    if t_sell is not None:
                        ...
                        if (t_sell == 0 or expected_future[0:t_sell].min() >= x0): 
                            ...
                            plt.scatter(tick, x0, marker='o' , c='g')
                    ...
                    if t_buy is not None:
                        ...
                        if (t_buy == 0 or expected_future[0:t_buy].max() <= x0):
                            have_state = False
                            actions[run,tick] = 'sell'
                            plt.scatter(tick, x0, marker='x', c='r' )
            # plt.show()
